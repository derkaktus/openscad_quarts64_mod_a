...
// These values are the max buildsize for boards and therefor are used for full size calculations. Only the mountpoints and cutouts are board or card specific.
    ...
    tr_support_x=100;     //Length of the support 
    tr_support_y=1.5;
    tr_support_h=30;
...
module x520s(){
    ...
    screw_support_x=3;
    screw_support_y=61;
    screw_support_h=tr_mount_out_h-0.5;             //Has to be even with screw mounts, so -0.5 to be seemless
    screw_support_offset_y=tr_y-screw_support_y-3;  //-3 was measured on my NIC ;)
    
    //Tray and mount 
    translate([0,screw_support_offset_y,tr_h])                              cube([screw_support_x,screw_support_y,tr_mount_out_h-0.5]); 
    translate([8.5,screw_support_offset_y+3.5,tr_h-0.5])                    screw_mount();
    translate([8.5,screw_support_offset_y+screw_support_y-7+3.5,tr_h-0.5])  screw_mount();

    // Creating base foot
    
    #translate([36.5,screw_support_offset_y+screw_support_y-7+3.5,tr_h-0.5]) cylinder(r=tr_mount_out_rad,h=tr_mount_out_h-0.5);     
    #translate([36.5,screw_support_offset_y+screw_support_y-7+3.5,tr_h-0.5])    
    ...
    //Shield
    difference(){
        translate([-shield_x,0,0])                                                                     cube([shield_x, tr_y, tr_support_h+tr_h]);
        // Cornered cuts
        //translate([-shield_x,screw_support_offset_y+31.5,screw_support_h+2])        cube([shield_x,2,3]);
        //translate([-shield_x,screw_support_offset_y+31.5+3.2,screw_support_h+2])    cube([shield_x,2,3]);
        // round cuts - whats translation adjustments as cylinders are centered

        hull(){
            translate([-shield_x,screw_support_offset_y+32.5,tr_h+screw_support_h+2])rotate([0,90,0])       cylinder(d=2,h=shield_x);
            translate([-shield_x,screw_support_offset_y+32.5,tr_h+screw_support_h+3])rotate([0,90,0])       cylinder(d=2,h=shield_x);
        }
        hull(){
            translate([-shield_x,screw_support_offset_y+32.5+3.2,tr_h+screw_support_h+2])rotate([0,90,0])   cylinder(d=2,h=shield_x);
            translate([-shield_x,screw_support_offset_y+32.5+3.2,tr_h+screw_support_h+3])rotate([0,90,0])   cylinder(d=2,h=shield_x);
        }
        #translate([-shield_x,screw_support_offset_y+31.5+2+1.2+2+1.2,tr_h+screw_support_h+1])                           cube([shield_x,15.5,10.35]);
}
}
