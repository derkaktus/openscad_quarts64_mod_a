...
if(make_dual){
  ...
  color("green")translate([shield_x,shm_y,0])tray(); 
  
  // Adding shield mounts with offset shield (x) - 1 to melt propperly  
  color("red")translate([shield_x-1,0,0])shield_mount();
...
// Main shield has two slots - one for the soc IO (shield_soc) one for the extra pcix card (shield_pcix).    
    ...
    if(make_dual){
        ...
        shield_main();
        mirror([0,0,1])shield_main();
        
        shield_soc();
        mirror([0,0,1])shield_soc();
        
        shield_connector();
        mirror([0,0,1])shield_connector();
    }
