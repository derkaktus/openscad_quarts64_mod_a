...
shield_y=88;
shield_h=88.40;
//Shield offset values to move the extra space around tray. y=0 means no space on the PCIx side of the board. Default is to even side spaces.
shield_offset_y=-((shield_y-(2*shm_y+tr_y))/2);
echo(shield_offset_y);
...
// Has no use if you set make_dual as space is used for second board
shield_addons_h=30;
...
make_short=1;
  //Set to 1 if you like to mount two boards behind one shield 
make_dual=1;  
...
if(make_dual){
    ...
    mirror([0,0,1])translate([shield_x,shm_y,0])tray();    
    translate([shield_x-1,0,0])shield_mount();
    mirror([0,0,1])translate([shield_x-1,0,0])shield_mount();
...
// Main shield has two slots - one for the soc IO (shield_soc) one for the extra pcix card (shield_pcix).    
    ...
    if(make_dual){
        ...
        shield_main();
        translate([0,shield_y+2*shield_offset_y,0])rotate([180,0,0])shield_main();
               
        shield_soc();
        translate([0,shield_y+2*shield_offset_y,0])rotate([180,0,0])shield_soc();
        
        shield_connector();
        translate([0,shield_y+2*shield_offset_y,0])rotate([180,0,0])shield_connector();
...
module shield_main(){
    //Creates the main shield with the rack screw holes + outcuts for soc / pcix shields 
        
        if(make_dual){
                difference(){
                    translate([0,shield_offset_y,0])    
                    difference(){
                        cube([shield_x, shield_y, shield_h/2]);
      
                        // Only two holes for the dual one as we mirror it on assmbley
                ...
                difference(){
                    ...
                    difference(){
                        ...
                        translate([0,shield_y/4,4+2])rotate([0,90,0])cylinder(h=shield_x,r=2);
                        translate([0,shield_y/4,shield_h-4+-2])rotate([0,90,0])cylinder(h=shield_x,r=2);
                        translate([0,shield_y-shield_y/4,4+2])rotate([0,90,0])cylinder(h=shield_x,r=2);
                        translate([0,shield_y-shield_y/4,shield_h-4+-2])rotate([0,90,0])cylinder(h=shield_x,r=2);
                    }
...
module shield_soc(){
    ...
    difference(){    
        ...
        // Power jack
        translate([0, shm_y+12.2,tr_h-0.5+sm_out_h+1.3])cube([shield_x, 10.0,8.2]);
        // NIC
        translate([0, shm_y+12.2+10.0+8.5,tr_h-0.5+sm_out_h+1.3]) cube([shield_x, 19.8,15.4]);
        // HDMI
        translate([0, shm_y+12.2+10.0+8.5+19.8+2.9,tr_h-0.5+sm_out_h+1.3])cube([shield_x, 16.1, 7.2]);     
    }
...
module shield_paint(){
// Creates extra onprint text for the shield. I didn t include it with the shield so it can be exchanged easier. Also I don t use mirror or rotate as reading direction for dual variant doesn t change. And dont forget to translate by shield_offset_y to match. Everything with x in minus area builds up on the shield.
    
    difference(){
        
        hull(){
                translate([-1,10,tr_h])rotate([0,90,0])cylinder(h=shield_x, r=1);
                translate([-1,10,tr_h+23])rotate([0,90,0])cylinder(h=shield_x, r=1);
                translate([-1,shield_y+2*shield_offset_y-11,tr_h])rotate([0,90,0])cylinder(h=shield_x, r=1);
                translate([-1,shield_y+2*shield_offset_y-11,tr_h+23])rotate([0,90,0])cylinder(h=shield_x, r=1);
        } 
        hull(){
                translate([-1,11,tr_h+1])rotate([0,90,0])cylinder(h=shield_x, r=1);
                translate([-1,11,tr_h+22])rotate([0,90,0])cylinder(h=shield_x, r=1);
                translate([-1,shield_y+2*shield_offset_y-12,tr_h+1])rotate([0,90,0])cylinder(h=shield_x, r=1);
                translate([-1,shield_y+2*shield_offset_y-12,tr_h+22])rotate([0,90,0])cylinder(h=shield_x, r=1);
        }
